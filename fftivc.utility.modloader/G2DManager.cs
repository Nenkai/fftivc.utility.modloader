...
public class G2DManager
{
    ...
    private Dictionary<FFTOGameMode, Dictionary<string /* locale */, G2DFileRegistry>> GameModeToModdedFiles { get; set; } = [];
    ...
    public void AddMapping(FFTOGameMode gameMode, string modIdOwner, uint fileIndex, string localFilePath, string? locale)
    {
        ...
        if (!GameModeToModdedFiles.TryGetValue(gameMode, out Dictionary<string, G2DFileRegistry>? g2dFilesPerLocale))
        {
            g2dFilesPerLocale = [];
            GameModeToModdedFiles.Add(gameMode, g2dFilesPerLocale);
        }

        if (!g2dFilesPerLocale.TryGetValue(locale, out G2DFileRegistry? g2dLocaleFile))
        {
            g2dLocaleFile = new G2DFileRegistry() { Locale = locale, ModdedFiles = [] };
            g2dFilesPerLocale.Add(locale, g2dLocaleFile);
        }

        if (g2dLocaleFile.ModdedFiles.TryGetValue(fileIndex, out G2DFileEntry? existingEntryForIndex))
        ...
        g2dLocaleFile.ModdedFiles[fileIndex] = new G2DFileEntry(modIdOwner, fileIndex, localFilePath);
    }
    ...
    /// <returns></returns>
    public unsafe int OnFetchG2DFile(uint fileIndex, nint bufferPtr)
    {
          if (GameModeToModdedFiles.TryGetValue(_currentGameMode, out Dictionary<string, G2DFileRegistry>? filesForGameMode) &&
            filesForGameMode.TryGetValue(_currentLocale, out G2DFileRegistry? localeRegistry) &&
            localeRegistry.ModdedFiles.TryGetValue(fileIndex, out G2DFileEntry? entry))
        {
            ...
            {
                fixed (byte* src = cachedBuffer)
                {
                    Buffer.MemoryCopy(src, (void*)bufferPtr, cachedBuffer.Length, cachedBuffer.Length);
                }
                return cachedBuffer.Length;
            }
        }

        // Not found so return -1, let the original function handle it.
        return -1;
    }
}

public class G2DFileRegistry
{
    public string? Locale { get; set; }
    public Dictionary<uint, G2DFileEntry> ModdedFiles { get; set; } = [];
}

public record G2DFileEntry(string ModIdOwner, uint FileIndex, string LocalFilePath);